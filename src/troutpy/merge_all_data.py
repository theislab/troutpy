...
def merge_all_sources(
    ...
    df_extracellular_counts = (
        df_extracellular.groupby("feature_name")
        .feature_name.count()
        .to_frame()
    ...
    df_intracellular_counts = (
        df_intracellular.groupby("feature_name")
        .feature_name.count()
        .to_frame()
    ...
    df_counts.loc[:, "fraction_extra_over_all"] = df_counts.counts_extracellular / (
        df_counts.counts_intracellular + df_counts.counts_extracellular
    )
    ...
    df_tmp = pd.merge(df23, df_counts, on="feature_name")
