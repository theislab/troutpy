...
def coupled_scatter(
    ...
    plt.scatter(
        transcripts[xcoord],
        transcripts[ycoord],
        c=transcripts[transcript_group],
        s=size * 0.1,
        cmap=colormap,
        vmax=vmax,
    )
...
def genes_over_noise(
    sdata, scores_by_genes, layer="extracellular_transcripts", output_path: str = "", save=True, format: str = "pdf"
):
    """
    ...
    if save == True:
        # Save the figure
        plt.savefig(
            os.path.join(PATH_FIGURES, f"boxplot_log_fold_change_per_gene{format}"), bbox_inches="tight", pad_inches=0
        )
    # Show the plot
    plt.show()
...
def nmf_factors_exrna_cells_W(
    sdata, nmf_adata_key="nmf_data", save=True, saving_path="", spot_size: int = 30, cmap="viridis"
):
...
def nmf_gene_contributions(
    sdata, nmf_adata_key="nmf_data", save=True, vmin=0.0, vmax=0.02, saving_path="", cmap="viridis", figsize=(5, 5)
):
    adata = sdata[nmf_adata_key]
...
def W(adata, n_factors, save=True):  # not very intuitive
    ...
    for factor in range(n_factors):
        sc.pl.spatial(
            ...
            title=f"NMF Factor {factor + 1}",
            spot_size=15,
            save=f"exo_to_cell_spatial_{factor}.png",
        )
...
def spatial_interactions(
    ...
    **kwargs,
    ...
    cell_positions = pd.DataFrame(
        sdata["table"].obsm[spatial_key], index=sdata.table.obs[cell_id_key], columns=["x", "y"]
    )
    ...
    plt.scatter(
        cell_positions.loc[target_cells, "x"],
        cell_positions.loc[target_cells, "y"],
        c=color_target,
        s=size,
        label="Target Cells",
        **kwargs,
    ...
    plt.scatter(
        cell_positions.loc[source_cells, "x"],
        cell_positions.loc[source_cells, "y"],
        c=color_source,
        s=size,
        label="Source Cells",
        **kwargs,
