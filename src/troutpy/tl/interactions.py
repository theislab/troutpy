...
def get_number_of_communication_genes(
    ...
    number_interactions_df = pd.DataFrame(
        index=source_binary.columns,
        columns=target_binary.columns
    )
    ...
    for col in source_binary.columns:
        ...
        for col2 in target_binary.columns:
            ...
            number_interactions_df.loc[col, col2] = len(
                set(sig_gene_source).intersection(sig_gene_target)
            )
