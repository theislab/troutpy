...
def segmentation_free_clustering(
    ...
    - 'num_clusters' (int)
        Number of clusters (default: 300).
    - 'pixel_width' (float)
        Pixel width parameter (default: 0.4).
    - 'pixel_smoothing' (float)
        Pixel smoothing parameter (default: 3.5).
    x (str)
    ...
    if method == "points2regions":
        ...
        p2r = Points2Regions(
            data[[x, y]], data[feature_name].astype(str), pixel_width=params["pixel_width"], pixel_smoothing=params["pixel_smoothing"]
        )
        p2r.fit(num_clusters=params["num_clusters"])

        # Retrieve clustering results
        adata = p2r._get_anndata()
        transcript_id_to_bin = dict(zip(adata.uns["reads"].index, adata.uns["reads"]["pixel_ind"], strict=False))
