...
def compute_source_cells(sdata, expression_threshold=1, gene_id_column="feature_name", layer="transcripts", copy=False):
    """
...
def distance_to_source_cell(
    ...
    for gene_of_interest in tqdm(adata_bin.var_names):
        ...
        extracellular_transcripts_filtered = extracellular_transcripts[
            extracellular_transcripts[gene_id_column] == gene_of_interest
        ].copy()
        ...
        if (adata_filtered.n_obs > 0) & (extracellular_transcripts_filtered.shape[0] > 0):
            ...
            transcript_coords = np.array(
                [extracellular_transcripts_filtered[xcoord], extracellular_transcripts_filtered[ycoord]]
            ).T
...
def compute_distant_cells_prop(sdata, layer="transcripts", gene_id_column="feature_name", threshold=30, copy=False):
    ...
    proportions_above_threshold = data.groupby(gene_id_column)["distance_to_source_cell"].apply(
